...
var Importer = function() {
  ...
  */
  var BackFiller = function (stopIndex, startIndex, callback) {
    ...
    if (startIndex < GENESIS_LEDGER) {
      console.log('start index precedes genesis ledger (' + GENESIS_LEDGER + ')');
      if (typeof callback === 'function') callback();
      return;
    }

    if (startIndex < stopIndex) {
      console.log('start index precedes stop index', stopIndex, startIndex);
      if (typeof callback === 'function') callback();
      return;
    }

    casinocoinAPI.connect().then(function() {
    ...
    function handleLedger (ledger) {
      ...
      //add it to the queue
        ...
        if (earliest === stopIndex) {
          console.log('backfill complete:', stopIndex, '-', startIndex);
          if (typeof callback === 'function') callback();
        }
      }
  ...
  */
  var LiveStream = function () {
    ...
    console.log("import: starting live stream");
    casinocoinAPI.connect();
    casinocoinAPI.on('ledger', function(resp) {
      if (!self._active) return;
      console.log('['+new Date().toISOString()+']', 'ledger closed:', resp.ledgerVersion);
    ...
    function handleLedger(ledger) {
      ...
      // check for gap
      if (current > latest + 1) {
        console.log("starting backfill:", latest + 1, '-', current - 1);
  ...
  self.getLedger = function (options, callback) {
    ...
    function requestLedger(options, callback) {
      ...
      console.log('['+new Date().toISOString()+']', 'requesting ledger:', options.ledgerVersion);
	console.log(options);
      casinocoinAPI.getLedger(options)
      .then(processLedger)
      .catch(function(e) {
      ...
      function processLedger(ledger) {
        ...
        try {
          hash = casinocoinAPI.computeLedgerHash(ledger);
        ...
        console.log('['+new Date().toISOString()+']', 'Got ledger: ' + ledger.ledgerVersion);
        callback(null, convertLedger(ledger));
      }
    }
  };
  ...
  */
  ...
  function retry(options, attempts, callback) {
    ...
    console.log("retry attempts:", options.attempts);
