...
var OffersExercised = function (tx) {
  ...
  if (tx.TransactionType !== 'Payment' && tx.TransactionType !== 'OfferCreate') {
    console.log(tx.TransactionType,'------------------------- tx.TransactionType')
    return list;
  }
  ...
  function parseOfferExercised (node, tx) {
    ...
    // TakerPays XRP
      ...
      base = {
        currency: 'XRP',
        amount: change.dividedBy(CSC_ADJUST).toString()
      }
      console.log(base,'----------------------else')
    }
    ...
    console.log(orderPair(offer), '-----------orderPair(offer)')
