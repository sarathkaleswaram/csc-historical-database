...
function BalanceChanges(tx) {
  ...
  function findType(data) {
    ...
    // offer creates are all exchanges
      ...
      // payment currency = CSC and destination account
      } else if (data.account === tx.Destination &&
                 !tx.Amount.currency &&
                 data.currency === 'CSC') {
        return 'payment_destination'
      ...
      // source currency = CSC and source account
      } else if (data.account === tx.Account &&
                tx.SendMax &&
                data.currency === 'CSC') {
        return 'payment_source'
  ...
  function parseAccountRoot(node) {
    ...
    if (tx.Account === account) {
      ...
      list.push({
        account: account,
        currency: 'CSC',
        change: fee.dividedBy(CSC_ADJUST).toString(),
        final_balance: balance.minus(amount).dividedBy(CSC_ADJUST).toString(),
    ...
    if (!amount.isZero()) {
      data = {
        account: account,
        currency: 'CSC',
        change: amount.dividedBy(CSC_ADJUST).toString(),
        final_balance: balance.dividedBy(CSC_ADJUST).toString(),
        time: tx.executed_time,
        ledger_index: tx.ledger_index,
        tx_index: tx.tx_index,
