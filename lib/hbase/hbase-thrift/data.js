...
HbaseClient.getKycTransactions = function(options, callback) {
  self.getScan({
    ...
    console.log(resp, '-----------------------resp getKycTransactions');
    if (resp) {
      resp.rows.forEach(function(row, i) {
        resp.rows[i] = row.tx_hash
      })
      options.hashes = resp.rows
      console.log(options.hashes, '----------------options.hashes')
    }
  })
  console.log(options.start.hbaseFormatStartRow(), '----------------options.start.hbaseFormatStartRow()')
  console.log(options.end.hbaseFormatStopRow(), '----------------options.end.hbaseFormatStopRow()')
  console.log(options.hashes, '----------------options.hashes')
  console.log(options, '----------------options')
  this.getTransactions(options, callback)
}
...
HbaseClient.getTransactions = function(options, callback) {
  ...
  function getTransactionsByTime(opts, cb) {
    ...
    console.log(opts, '-----------------------opts getTransactionsByTime');
    ...
    self.getScanWithMarker(self, {
      ...
      console.log(resp, '-----------------------resp getTransactionsByTime');
      if (resp) {
        resp.rows.forEach(function(row, i) {
          resp.rows[i] = row.tx_hash
        })
  ...
  function getTransactionsFromHashes(opts, cb) {
    ...
    function formatTx(d) {
      ...
      if (opts.include_ledger_hash) {
        console.log('coming include ledger hash');
  ...
  if (options.hashes) {
    ...
    getTransactionsByTime(options, function(err, resp) {
      ...
      if (err) {
        ...
        options.hashes = resp.rows
        console.log(options, '-------------------options getTransactionsByTime cb');
        getTransactionsFromHashes(options, callback)
...
HbaseClient.saveTransactions = function(transactions, callback) {
  ...
  console.log(tables, '----------------------saveTransactions');
...
HbaseClient.prepareParsedData = function(data) {
  ...
  data.kycTransactions.forEach(function(tx) {
    // transactions by time
    var key = utils.formatTime(tx.executed_time) +
      '|' + utils.padNumber(tx.ledger_index, LI_PAD) +
      '|' + utils.padNumber(tx.tx_index, I_PAD)

    tables.kyc_transactions[key] = {
      tx_hash: tx.tx_hash,
      tx_index: tx.tx_index,
      'f:ledger_index': tx.ledger_index,
      'f:type': tx.type,
  ...
  console.log(data.kycTransactions, '-----------------------data.kycTransactions')
...
HbaseClient.saveParsedData = function(params, callback) {
  ...
  console.log(tables, '------------------------saveParsedData')
  var tableNames
