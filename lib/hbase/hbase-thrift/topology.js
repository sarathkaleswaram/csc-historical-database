...
function getTopologyNodes(options) {
  ...
  function getNodeDetails(pubkey) {
    console.log(pubkey,'-----------------pubkey getNodeDetails');
  ...
  function getNodeStats(pubkey) {
    console.log(pubkey,'-----------------pubkey getNodeStats');
  ...
  .then(function(info) {
    return new Promise(function(resolve, reject) {
      ...
      self.getScanWithMarker(self, {
        ...
        console.log(resp, '----------------resp getScanWithMarker');

        if (err) {
          reject(err)

        } else if (options.details) {
          console.log(options.details, '--------------------options.details');
          ...
          console.log(resp.rows.map(formatNodeStats), '----------------resp.rows.map(formatNodeStats)');
  ...
  return new Promise(function(resolve, reject) {
    self.getScan({
      ...
      if (err) {
        ...
        console.log(resp && resp[0] ? resp[0] : null, '-------------------else');
