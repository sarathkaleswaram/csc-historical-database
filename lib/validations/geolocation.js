...
module.exports = config => {
  ...
  const cf = config.columnFamily || 'd';
  ...
  function updateGeolocation(node) {
    ...
    console.log(url, '----------------------url updateGeolocation')
    ...
    .then(resp => {
      ...
      console.log( table, '-------------table', node.node_public_key, '-------------node_public_key', columns, '-------------columns')
  ...
  return {
    geolocateNodes: function() {
      ...
      .then(nodes => {
        console.log(nodes, '--------------------------nodes geolocation');
