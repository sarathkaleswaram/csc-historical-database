...
var importer = require('./casinocoin-importer');
...
function LedgerStream() {
  ...
  validator.on('ledger', function(ledger, callback) {
    console.log(ledger,'-------------------- validator on ledger')
    self.ledgers.push({
      ledger: ledger,
      cb: callback
    });
    this.log.error('validator--------------------------self.ledgers',self.ledgers)
  });

  importer.on('ledger', function(ledger) {
    console.log(ledger,'-------------------- importer on ledger')
    self.ledgers.push({
      ledger: ledger,
      cb: null
    });
    this.log.info('importer--------------------------self.ledgers',self.ledgers)
  });
};

//start live importer
LedgerStream.prototype.start = function () {
  console.log('-----------------------LedgerStream.prototype.start')
  validator.start();
  importer.liveStream();
};
...
LedgerStream.prototype.processNextLedger = function (callback) {
  console.log('--------------------- processNextLedger')
  var self   = this;
  var row    = this.ledgers.shift();
  var transactions;
  console.log(row, '------------------------ row processNextLedger before')
  ...
  console.log(row, '------------------------ row processNextLedger after')
...
LedgerStream.prototype.prepareTransaction = function (ledger, tx) {
  ...
  console.log(tx, '--------------- tx LedgerStream.prototype.prepareTransaction')
  return tx;
};
